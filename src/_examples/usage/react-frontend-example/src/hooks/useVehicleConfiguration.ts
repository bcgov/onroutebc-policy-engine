...
export const useVehicleConfiguration = () => {
  ...
  const updateAxleConfigurationsForTrailers = useCallback((trailerCount: number) => {
  ...
  const handleTrailerChange = useCallback((index: number, value: string) => {
  ...
  const removeTrailer = useCallback((index: number) => {
    setSelectedTrailers(prev => {
      const newTrailers = [...prev]
  ...
  const handleAxleConfigurationChange = useCallback((axleIndex: number, field: string, value: string) => {
    setAxleConfigurations(prev => {
      const newConfigs = [...prev]
  ...
  const setTrailersFromData = useCallback((trailers: any[]) => {
    const trailerTypes = trailers.map((trailer: any) => trailer.vehicleSubType)
