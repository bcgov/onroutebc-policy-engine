...
export const useFormWatchers = ({
  ...
  useEffect(() => {
    if (watchedPermitType && policy) {
      try {
        ...
        setShowSizeDimensions(
          permitTypeDefinition?.sizeDimensionRequired || false,
        );
        setShowWeightDimensions(
          permitTypeDefinition?.weightDimensionRequired || false,
        );
  ...
  useEffect(() => {
    if (
      watchedVehicleCountryCode === 'MX' ||
      watchedVehicleCountryCode === 'XX'
    ) {
