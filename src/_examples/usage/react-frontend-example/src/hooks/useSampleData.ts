    ...
    async (permitType: string) => {
      ...
      try {
        ...
        const newFormData: Partial<PermitFormSchema> = {
          ...
          loadedGVW:
            permitData.vehicleConfiguration?.loadedGVW?.toString() || '',
          netWeight:
            permitData.vehicleConfiguration?.netWeight?.toString() || '',
          ...
          // Permitted Route
          highwaySequence:
            permitData.permittedRoute?.manualRoute?.highwaySequence?.join(
              ', ',
            ) || '',
          ...
          routeExitPoint:
            permitData.permittedRoute?.manualRoute?.exitPoint || '',
          routeTotalDistance:
            permitData.permittedRoute?.manualRoute?.totalDistance?.toString() ||
            '',

          // Vehicle Configuration
          overallLength:
            permitData.vehicleConfiguration?.overallLength?.toString() || '',
          overallWidth:
            permitData.vehicleConfiguration?.overallWidth?.toString() || '',
          overallHeight:
            permitData.vehicleConfiguration?.overallHeight?.toString() || '',
          frontProjection:
            permitData.vehicleConfiguration?.frontProjection?.toString() || '',
          rearProjection:
            permitData.vehicleConfiguration?.rearProjection?.toString() || '',
        ...
        if (permitData.vehicleDetails?.vehicleType && policy) {
          try {
            const subTypes = getVehicleSubTypes(
