import { PolicyDefinition } from "onroute-policy-engine/types";

export const data: PolicyDefinition = {
  "minPEVersion": "2.0.0",
  "geographicRegions": [
    {
      "id": "LMN",
      "name": "Lower Mainland"
    },
    {
      "id": "ELK",
      "name": "Elk Valley"
    },
    {
      "id": "YHO",
      "name": "Yoho"
    },
    {
      "id": "PCE",
      "name": "Peace"
    }
  ],
  "rangeMatrices": [
    {
      "id": "annualFeeCV",
      "name": "Annual licensing fee for commercial vehicle",
      "matrix": [
        {
          "min": 0,
          "max": 500,
          "value": 42
        },
        {
          "min": 501,
          "max": 1000,
          "value": 49
        },
        {
          "min": 1001,
          "max": 1500,
          "value": 60
        },
        {
          "min": 1501,
          "max": 2000,
          "value": 74
        },
        {
          "min": 2001,
          "max": 2500,
          "value": 85
        },
        {
          "min": 2501,
          "max": 3000,
          "value": 97
        },
        {
          "min": 3001,
          "max": 3500,
          "value": 108
        },
        {
          "min": 3501,
          "max": 4000,
          "value": 127
        },
        {
          "min": 4001,
          "max": 4500,
          "value": 147
        },
        {
          "min": 4501,
          "max": 5000,
          "value": 173
        },
        {
          "min": 5001,
          "max": 5500,
          "value": 193
        },
        {
          "min": 5501,
          "max": 6000,
          "value": 213
        },
        {
          "min": 6001,
          "max": 6500,
          "value": 230
        },
        {
          "min": 6501,
          "max": 7000,
          "value": 250
        },
        {
          "min": 7001,
          "max": 7500,
          "value": 266
        },
        {
          "min": 7501,
          "max": 8000,
          "value": 292
        },
        {
          "min": 8001,
          "max": 8500,
          "value": 320
        },
        {
          "min": 8501,
          "max": 9000,
          "value": 347
        },
        {
          "min": 9001,
          "max": 9500,
          "value": 376
        },
        {
          "min": 9501,
          "max": 10000,
          "value": 395
        },
        {
          "min": 10001,
          "max": 10500,
          "value": 416
        },
        {
          "min": 10501,
          "max": 11000,
          "value": 437
        },
        {
          "min": 11001,
          "max": 11500,
          "value": 450
        },
        {
          "min": 11501,
          "max": 12000,
          "value": 469
        },
        {
          "min": 12001,
          "max": 12500,
          "value": 493
        },
        {
          "min": 12501,
          "max": 13000,
          "value": 507
        },
        {
          "min": 13001,
          "max": 13500,
          "value": 532
        },
        {
          "min": 13501,
          "max": 14000,
          "value": 559
        },
        {
          "min": 14001,
          "max": 14500,
          "value": 586
        },
        {
          "min": 14501,
          "max": 15000,
          "value": 613
        },
        {
          "min": 15001,
          "max": 15500,
          "value": 645
        },
        {
          "min": 15501,
          "max": 16000,
          "value": 687
        },
        {
          "min": 16001,
          "max": 16500,
          "value": 729
        },
        {
          "min": 16501,
          "max": 17000,
          "value": 769
        },
        {
          "min": 17001,
          "max": 17500,
          "value": 814
        },
        {
          "min": 17501,
          "max": 18000,
          "value": 846
        },
        {
          "min": 18001,
          "max": 18500,
          "value": 870
        },
        {
          "min": 18501,
          "max": 19000,
          "value": 899
        },
        {
          "min": 19001,
          "max": 19500,
          "value": 927
        },
        {
          "min": 19501,
          "max": 20000,
          "value": 954
        },
        {
          "min": 20001,
          "max": 20500,
          "value": 987
        },
        {
          "min": 20501,
          "max": 21000,
          "value": 1014
        },
        {
          "min": 21001,
          "max": 21500,
          "value": 1041
        },
        {
          "min": 21501,
          "max": 22000,
          "value": 1068
        },
        {
          "min": 22001,
          "max": 22500,
          "value": 1095
        },
        {
          "min": 22501,
          "max": 23000,
          "value": 1123
        },
        {
          "min": 23001,
          "max": 23500,
          "value": 1152
        },
        {
          "min": 23501,
          "max": 24000,
          "value": 1182
        },
        {
          "min": 24001,
          "max": 24500,
          "value": 1212
        },
        {
          "min": 24501,
          "max": 25000,
          "value": 1252
        },
        {
          "min": 25001,
          "max": 25500,
          "value": 1298
        },
        {
          "min": 25501,
          "max": 26000,
          "value": 1340
        },
        {
          "min": 26001,
          "max": 26500,
          "value": 1381
        },
        {
          "min": 26501,
          "max": 27000,
          "value": 1410
        },
        {
          "min": 27001,
          "max": 27500,
          "value": 1439
        },
        {
          "min": 27501,
          "max": 28000,
          "value": 1465
        },
        {
          "min": 28001,
          "max": 28500,
          "value": 1495
        },
        {
          "min": 28501,
          "max": 29000,
          "value": 1521
        },
        {
          "min": 29001,
          "max": 29500,
          "value": 1550
        },
        {
          "min": 29501,
          "max": 30000,
          "value": 1581
        },
        {
          "min": 30001,
          "max": 31000,
          "value": 1608
        },
        {
          "min": 31001,
          "max": 32000,
          "value": 1661
        },
        {
          "min": 32001,
          "max": 33000,
          "value": 1714
        },
        {
          "min": 33001,
          "max": 34000,
          "value": 1769
        },
        {
          "min": 34001,
          "max": 35000,
          "value": 1824
        },
        {
          "min": 35001,
          "max": 36000,
          "value": 1910
        },
        {
          "min": 36001,
          "max": 37000,
          "value": 2039
        },
        {
          "min": 37001,
          "max": 38000,
          "value": 2110
        },
        {
          "min": 38001,
          "max": 39000,
          "value": 2182
        },
        {
          "min": 39001,
          "max": 40000,
          "value": 2252
        },
        {
          "min": 40001,
          "max": 41000,
          "value": 2324
        },
        {
          "min": 41001,
          "max": 42000,
          "value": 2398
        },
        {
          "min": 42001,
          "max": 43000,
          "value": 2471
        },
        {
          "min": 43001,
          "max": 44000,
          "value": 2540
        },
        {
          "min": 44001,
          "max": 45000,
          "value": 2612
        },
        {
          "min": 45001,
          "max": 46000,
          "value": 2718
        },
        {
          "min": 46001,
          "max": 47000,
          "value": 2828
        },
        {
          "min": 47001,
          "max": 48000,
          "value": 2901
        },
        {
          "min": 48001,
          "max": 49000,
          "value": 2971
        },
        {
          "min": 49001,
          "max": 50000,
          "value": 3044
        },
        {
          "min": 50001,
          "max": 51000,
          "value": 3093
        },
        {
          "min": 51001,
          "max": 52000,
          "value": 3160
        },
        {
          "min": 52001,
          "max": 53000,
          "value": 3226
        },
        {
          "min": 53001,
          "max": 54000,
          "value": 3291
        },
        {
          "min": 54001,
          "max": 55000,
          "value": 3357
        },
        {
          "min": 55001,
          "max": 56000,
          "value": 3423
        },
        {
          "min": 56001,
          "max": 57000,
          "value": 3488
        },
        {
          "min": 57001,
          "max": 58000,
          "value": 3553
        },
        {
          "min": 58001,
          "max": 59000,
          "value": 3619
        },
        {
          "min": 59001,
          "max": 60000,
          "value": 3685
        },
        {
          "min": 60001,
          "max": 61000,
          "value": 3749
        },
        {
          "min": 61001,
          "max": 62000,
          "value": 3815
        },
        {
          "min": 62001,
          "max": 63000,
          "value": 3880
        },
        {
          "min": 63001,
          "max": 63500,
          "value": 3946
        }
      ]
    },
    {
      "id": "annualFeePassenger",
      "name": "Annual licensing fee for commercial passenger vehicle",
      "matrix": [
        {
          "min": 0,
          "max": 500,
          "value": 40
        },
        {
          "min": 501,
          "max": 1000,
          "value": 47
        },
        {
          "min": 1001,
          "max": 1500,
          "value": 57
        },
        {
          "min": 1501,
          "max": 2000,
          "value": 70
        },
        {
          "min": 2001,
          "max": 2500,
          "value": 81
        },
        {
          "min": 2501,
          "max": 3000,
          "value": 92
        },
        {
          "min": 3001,
          "max": 3500,
          "value": 103
        },
        {
          "min": 3501,
          "max": 4000,
          "value": 121
        },
        {
          "min": 4001,
          "max": 4500,
          "value": 140
        },
        {
          "min": 4501,
          "max": 5000,
          "value": 165
        },
        {
          "min": 5001,
          "max": 5500,
          "value": 184
        },
        {
          "min": 5501,
          "max": 6000,
          "value": 203
        },
        {
          "min": 6001,
          "max": 6500,
          "value": 219
        },
        {
          "min": 6501,
          "max": 7000,
          "value": 238
        },
        {
          "min": 7001,
          "max": 7500,
          "value": 253
        },
        {
          "min": 7501,
          "max": 8000,
          "value": 278
        },
        {
          "min": 8001,
          "max": 8500,
          "value": 305
        },
        {
          "min": 8501,
          "max": 9000,
          "value": 330
        },
        {
          "min": 9001,
          "max": 9500,
          "value": 358
        },
        {
          "min": 9501,
          "max": 10000,
          "value": 376
        },
        {
          "min": 10001,
          "max": 10500,
          "value": 396
        },
        {
          "min": 10501,
          "max": 11000,
          "value": 416
        },
        {
          "min": 11001,
          "max": 11500,
          "value": 429
        },
        {
          "min": 11501,
          "max": 12000,
          "value": 447
        },
        {
          "min": 12001,
          "max": 12500,
          "value": 465
        },
        {
          "min": 12501,
          "max": 13000,
          "value": 478
        },
        {
          "min": 13001,
          "max": 13500,
          "value": 501
        },
        {
          "min": 13501,
          "max": 14000,
          "value": 527
        },
        {
          "min": 14001,
          "max": 14500,
          "value": 552
        },
        {
          "min": 14501,
          "max": 15000,
          "value": 578
        },
        {
          "min": 15001,
          "max": 15500,
          "value": 608
        },
        {
          "min": 15501,
          "max": 16000,
          "value": 648
        },
        {
          "min": 16001,
          "max": 16500,
          "value": 687
        },
        {
          "min": 16501,
          "max": 17000,
          "value": 725
        },
        {
          "min": 17001,
          "max": 17500,
          "value": 768
        },
        {
          "min": 17501,
          "max": 18000,
          "value": 797
        },
        {
          "min": 18001,
          "max": 18500,
          "value": 820
        },
        {
          "min": 18501,
          "max": 19000,
          "value": 848
        },
        {
          "min": 19001,
          "max": 19500,
          "value": 873
        },
        {
          "min": 19501,
          "max": 20000,
          "value": 899
        },
        {
          "min": 20001,
          "max": 20500,
          "value": 930
        },
        {
          "min": 20501,
          "max": 21000,
          "value": 955
        },
        {
          "min": 21001,
          "max": 21500,
          "value": 981
        },
        {
          "min": 21501,
          "max": 22000,
          "value": 1007
        },
        {
          "min": 22001,
          "max": 22500,
          "value": 1032
        },
        {
          "min": 22501,
          "max": 23000,
          "value": 1058
        },
        {
          "min": 23001,
          "max": 23500,
          "value": 1086
        },
        {
          "min": 23501,
          "max": 24000,
          "value": 1114
        },
        {
          "min": 24001,
          "max": 24500,
          "value": 1142
        },
        {
          "min": 24501,
          "max": 25000,
          "value": 1180
        },
        {
          "min": 25001,
          "max": 25500,
          "value": 1224
        },
        {
          "min": 25501,
          "max": 26000,
          "value": 1263
        },
        {
          "min": 26001,
          "max": 26500,
          "value": 1302
        },
        {
          "min": 26501,
          "max": 27000,
          "value": 1329
        },
        {
          "min": 27001,
          "max": 27500,
          "value": 1356
        },
        {
          "min": 27501,
          "max": 28000,
          "value": 1381
        },
        {
          "min": 28001,
          "max": 28500,
          "value": 1409
        },
        {
          "min": 28501,
          "max": 29000,
          "value": 1433
        },
        {
          "min": 29001,
          "max": 29500,
          "value": 1461
        },
        {
          "min": 29501,
          "max": 30000,
          "value": 1490
        },
        {
          "min": 30001,
          "max": 31000,
          "value": 1516
        },
        {
          "min": 31001,
          "max": 32000,
          "value": 1569
        },
        {
          "min": 32001,
          "max": 33000,
          "value": 1621
        },
        {
          "min": 33001,
          "max": 34000,
          "value": 1676
        },
        {
          "min": 34001,
          "max": 35000,
          "value": 1730
        },
        {
          "min": 35001,
          "max": 36000,
          "value": 1815
        },
        {
          "min": 36001,
          "max": 37000,
          "value": 1943
        },
        {
          "min": 37001,
          "max": 38000,
          "value": 2013
        },
        {
          "min": 38001,
          "max": 39000,
          "value": 2084
        },
        {
          "min": 39001,
          "max": 40000,
          "value": 2154
        },
        {
          "min": 40001,
          "max": 41000,
          "value": 2225
        },
        {
          "min": 41001,
          "max": 42000,
          "value": 2298
        },
        {
          "min": 42001,
          "max": 43000,
          "value": 2370
        },
        {
          "min": 43001,
          "max": 44000,
          "value": 2439
        },
        {
          "min": 44001,
          "max": 45000,
          "value": 2510
        },
        {
          "min": 45001,
          "max": 46000,
          "value": 2615
        },
        {
          "min": 46001,
          "max": 47000,
          "value": 2724
        },
        {
          "min": 47001,
          "max": 48000,
          "value": 2796
        },
        {
          "min": 48001,
          "max": 49000,
          "value": 2865
        },
        {
          "min": 49001,
          "max": 50000,
          "value": 2937
        },
        {
          "min": 50001,
          "max": 51000,
          "value": 2986
        },
        {
          "min": 51001,
          "max": 52000,
          "value": 3052
        },
        {
          "min": 52001,
          "max": 53000,
          "value": 3117
        },
        {
          "min": 53001,
          "max": 54000,
          "value": 3182
        },
        {
          "min": 54001,
          "max": 55000,
          "value": 3247
        },
        {
          "min": 55001,
          "max": 56000,
          "value": 3312
        },
        {
          "min": 56001,
          "max": 57000,
          "value": 3377
        },
        {
          "min": 57001,
          "max": 58000,
          "value": 3441
        },
        {
          "min": 58001,
          "max": 59000,
          "value": 3506
        },
        {
          "min": 59001,
          "max": 60000,
          "value": 3572
        },
        {
          "min": 60001,
          "max": 61000,
          "value": 3635
        },
        {
          "min": 61001,
          "max": 62000,
          "value": 3700
        },
        {
          "min": 62001,
          "max": 63000,
          "value": 3765
        },
        {
          "min": 63001,
          "max": 63500,
          "value": 3830
        }
      ]
    },
    {
      "id": "annualFeeIndustrial",
      "name": "Annual licensing fee for an industrial machine",
      "matrix": [
        {
          "min": 0,
          "max": 2000,
          "value": 45
        },
        {
          "min": 2001,
          "max": 5000,
          "value": 69
        },
        {
          "min": 5001,
          "max": 7000,
          "value": 110
        },
        {
          "min": 7001,
          "max": 9000,
          "value": 164
        },
        {
          "min": 9001,
          "max": 11000,
          "value": 216
        },
        {
          "min": 11001,
          "value": 260
        }
      ]
    },
    {
      "id": "annualFeeFarm",
      "name": "Annual licensing fee for farm vehicle",
      "matrix": [
        {
          "min": 0,
          "max": 500,
          "value": 30
        },
        {
          "min": 501,
          "max": 1000,
          "value": 40
        },
        {
          "min": 1001,
          "max": 1500,
          "value": 47
        },
        {
          "min": 1501,
          "max": 2000,
          "value": 55
        },
        {
          "min": 2001,
          "max": 2500,
          "value": 77
        },
        {
          "min": 2501,
          "max": 3000,
          "value": 101
        },
        {
          "min": 3001,
          "max": 3500,
          "value": 142
        },
        {
          "min": 3501,
          "max": 4000,
          "value": 181
        },
        {
          "min": 4001,
          "max": 4500,
          "value": 207
        },
        {
          "min": 4501,
          "max": 5000,
          "value": 243
        },
        {
          "min": 5001,
          "max": 5500,
          "value": 278
        },
        {
          "min": 5501,
          "max": 6000,
          "value": 322
        },
        {
          "min": 6001,
          "max": 6500,
          "value": 355
        },
        {
          "min": 6501,
          "max": 7000,
          "value": 396
        },
        {
          "min": 7001,
          "max": 7500,
          "value": 427
        },
        {
          "min": 7501,
          "max": 8000,
          "value": 473
        },
        {
          "min": 8001,
          "max": 8500,
          "value": 524
        },
        {
          "min": 8501,
          "max": 9000,
          "value": 558
        },
        {
          "min": 9001,
          "max": 9500,
          "value": 596
        },
        {
          "min": 9501,
          "max": 10000,
          "value": 633
        },
        {
          "min": 10001,
          "max": 10500,
          "value": 669
        },
        {
          "min": 10501,
          "max": 11000,
          "value": 711
        },
        {
          "min": 11001,
          "max": 11500,
          "value": 744
        },
        {
          "min": 11501,
          "max": 12000,
          "value": 784
        },
        {
          "min": 12001,
          "max": 12500,
          "value": 824
        },
        {
          "min": 12501,
          "max": 13000,
          "value": 863
        },
        {
          "min": 13001,
          "max": 13500,
          "value": 883
        },
        {
          "min": 13501,
          "max": 14000,
          "value": 899
        },
        {
          "min": 14001,
          "max": 14500,
          "value": 919
        },
        {
          "min": 14501,
          "max": 15000,
          "value": 940
        },
        {
          "min": 15001,
          "max": 15500,
          "value": 960
        },
        {
          "min": 15501,
          "max": 16000,
          "value": 979
        },
        {
          "min": 16001,
          "max": 16500,
          "value": 997
        },
        {
          "min": 16501,
          "max": 17000,
          "value": 1017
        },
        {
          "min": 17001,
          "max": 17500,
          "value": 1036
        },
        {
          "min": 17501,
          "max": 18000,
          "value": 1056
        },
        {
          "min": 18001,
          "max": 18500,
          "value": 1076
        },
        {
          "min": 18501,
          "max": 19000,
          "value": 1096
        },
        {
          "min": 19001,
          "max": 19500,
          "value": 1114
        },
        {
          "min": 19501,
          "max": 20000,
          "value": 1134
        },
        {
          "min": 20001,
          "max": 20500,
          "value": 1154
        },
        {
          "min": 20501,
          "max": 21000,
          "value": 1174
        },
        {
          "min": 21001,
          "max": 21500,
          "value": 1192
        },
        {
          "min": 21501,
          "max": 22000,
          "value": 1211
        },
        {
          "min": 22001,
          "max": 22500,
          "value": 1231
        },
        {
          "min": 22501,
          "max": 23000,
          "value": 1251
        },
        {
          "min": 23001,
          "max": 23500,
          "value": 1270
        },
        {
          "min": 23501,
          "max": 24000,
          "value": 1289
        },
        {
          "min": 24001,
          "max": 24400,
          "value": 1309
        }
      ]
    }
  ],
  "bridgeCalculationConstants": {
    "minWeight": 18000,
    "multiplier": 30
  },
  "commonRules": [
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "companyName",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Company is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.companyName"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "contactDetails.firstName",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Contact first name is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.contactDetails.firstName"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "contactDetails.lastName",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Contact last name is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.contactDetails.lastName"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "contactDetails.phone1",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Contact phone number is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.contactDetails.phone1"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "contactDetails.email",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Company contact email is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.contactDetails.email"
        }
      }
    },
    {
      "conditions": {
        "any": [
          {
            "fact": "permitData",
            "path": "$.startDate",
            "operator": "dateLessThan",
            "value": {
              "fact": "validationDate"
            }
          }
        ]
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Permit start date cannot be in the past",
          "code": "field-validation-error",
          "fieldReference": "permitData.startDate"
        }
      }
    },
    {
      "conditions": {
        "any": [
          {
            "fact": "daysBetween",
            "operator": "greaterThan",
            "value": 14,
            "params": {
              "dateFrom": {
                "fact": "validationDate"
              },
              "dateTo": {
                "fact": "permitData",
                "path": "$.startDate"
              }
            }
          }
        ]
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Permit start date cannot be more than 14 days in the future",
          "code": "field-validation-error",
          "fieldReference": "permitData.startDate"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "vehicleDetails.vin",
          "operator": "regex",
          "value": "^[a-zA-Z0-9]{6}$"
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Vehicle Identification Number (vin) must be 6 alphanumeric characters",
          "code": "field-validation-error",
          "fieldReference": "permitData.vehicleDetails.vin"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "vehicleDetails.plate",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Vehicle plate is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.vehicleDetails.plate"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "vehicleDetails.make",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Vehicle make is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.vehicleDetails.make"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "vehicleDetails.year",
          "operator": "greaterThan",
          "value": 1900
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Vehicle year is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.vehicleDetails.year"
        }
      }
    },
    {
      "conditions": {
        "not": {
          "fact": "permitData",
          "path": "vehicleDetails.countryCode",
          "operator": "stringMinimumLength",
          "value": 1
        }
      },
      "event": {
        "type": "violation",
        "params": {
          "message": "Vehicle country of registration is required",
          "code": "field-validation-error",
          "fieldReference": "permitData.vehicleDetails.countryCode"
        }
      }
    }
  ],
  "permitTypes": [
    {
      "id": "TROS",
      "name": "Term Oversize",
      "routingRequired": false,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "DDCKBUS",
        "GRADERS",
        "BUSTRLR",
        "LOGGING",
        "LOGOFFH",
        "LCVTPDB",
        "LCVRMDB",
        "MUNFITR",
        "OGBEDTK",
        "OGOILSW",
        "OGSERVC",
        "PICKRTT",
        "PICKRTR",
        "RBTRLDR",
        "TLSCONV",
        "TOWVEHC",
        "TRKTRAC",
        "STINGER",
        "REGTRCK",
        "PLOWBLD",
        "DOLLIES",
        "EXPANDO",
        "FECVYRX",
        "FEDRMMX",
        "FEPNYTR",
        "FEBGHSE",
        "FESEMTR",
        "FEWHELR",
        "FEWHLPR",
        "FLOATTR",
        "FULLLTL",
        "LOGNTAC",
        "LOGTNDM",
        "LOGTRIX",
        "LOGDGLG",
        "LOGSMEM",
        "LOGFULL",
        "LOGOWBK",
        "OGOSFDT",
        "ODTRLEX",
        "PLATFRM",
        "POLETRL",
        "PONYTRL",
        "REDIMIX",
        "SEMITRL",
        "HIBOEXP",
        "HIBOFLT",
        "STREEFR",
        "STLOGNG",
        "STNTSHC",
        "STSDBDK",
        "STSTNGR",
        "STWHELR",
        "STWIDWH",
        "STCRANE",
        "STINGAT"
      ],
      "rules": [
        {
          "conditions": {
            "all": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "in",
                  "value": [
                    30,
                    60,
                    90,
                    120,
                    150,
                    180,
                    210,
                    240,
                    270,
                    300,
                    330
                  ]
                }
              },
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "equal",
                  "value": {
                    "fact": "daysInPermitYear"
                  }
                }
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Duration must be in 30 day increments or a full year",
              "code": "field-validation-error",
              "fieldReference": "permitData.permitDuration"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "costPerMonth",
          "params": {
            "cost": 30
          }
        }
      ],
      "conditions": [
        {
          "condition": "CVSE-1000",
          "mandatory": true
        },
        {
          "condition": "CVSE-1070",
          "mandatory": true
        },
        {
          "condition": "CVSE-1000S",
          "mandatory": false
        },
        {
          "condition": "CVSE-1000L",
          "mandatory": false
        },
        {
          "condition": "CVSE-1012",
          "mandatory": false
        },
        {
          "condition": "CVSE-1013",
          "mandatory": false
        }
      ]
    },
    {
      "id": "TROW",
      "name": "Term Overweight",
      "routingRequired": false,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "GRADERS",
        "MUNFITR",
        "LCVTPDB",
        "LCVRMDB",
        "OGOILSW",
        "OGSERVC",
        "PICKRTT",
        "PICKRTR",
        "RBTRLDR",
        "TLSCONV",
        "TOWVEHC",
        "TRKTRAC",
        "FECVYRX",
        "FEDRMMX",
        "FEBGHSE",
        "FESEMTR",
        "FEWHELR",
        "FEWHLPR",
        "REDIMIX"
      ],
      "rules": [
        {
          "conditions": {
            "all": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "in",
                  "value": [
                    30,
                    60,
                    90,
                    120,
                    150,
                    180,
                    210,
                    240,
                    270,
                    300,
                    330
                  ]
                }
              },
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "equal",
                  "value": {
                    "fact": "daysInPermitYear"
                  }
                }
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Duration must be in 30 day increments or a full year",
              "code": "field-validation-error",
              "fieldReference": "permitData.permitDuration"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "costPerMonth",
          "params": {
            "cost": 100
          }
        }
      ],
      "conditions": [
        {
          "condition": "CVSE-1000",
          "mandatory": true
        },
        {
          "condition": "CVSE-1070",
          "mandatory": true
        },
        {
          "condition": "CVSE-1011",
          "mandatory": true
        }
      ]
    },
    {
      "id": "STOS",
      "name": "Single Trip Oversize",
      "routingRequired": true,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": true,
      "commodityRequired": true,
      "rules": [
        {
          "conditions": {
            "any": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "lessThanInclusive",
                  "value": 7
                }
              },
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "greaterThan",
                  "value": 0
                }
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Duration must be 7 days or less",
              "code": "field-validation-error",
              "fieldReference": "permitData.permitDuration"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "configurationIsValid",
              "operator": "equal",
              "value": true
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle configuration is not permittable for this commodity",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.trailers"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "permittedCommodity.loadDescription",
              "operator": "stringMinimumLength",
              "value": 1
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Commodity load description is required",
              "code": "field-validation-error",
              "fieldReference": "permitData.permittedCommodity.loadDescription"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "fixedCost",
          "params": {
            "cost": 15
          }
        }
      ],
      "conditions": [
        {
          "condition": "CVSE-1000",
          "mandatory": true
        },
        {
          "condition": "CVSE-1070",
          "mandatory": true
        },
        {
          "condition": "CVSE-1001",
          "mandatory": false
        },
        {
          "condition": "CVSE-1002",
          "mandatory": false
        },
        {
          "condition": "CVSE-1010",
          "mandatory": false
        },
        {
          "condition": "CVSE-1012",
          "mandatory": false
        },
        {
          "condition": "CVSE-1013",
          "mandatory": false
        }
      ]
    },
    {
      "id": "STOW",
      "name": "Single Trip Overweight",
      "routingRequired": true,
      "weightDimensionRequired": true,
      "sizeDimensionRequired": false,
      "commodityRequired": true,
      "rules": [
        {
          "conditions": {
            "not": {
              "fact": "configurationIsValid",
              "operator": "equal",
              "value": true
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle configuration is not permittable for this commodity",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.trailers"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "permittedCommodity.loadDescription",
              "operator": "stringMinimumLength",
              "value": 1
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Commodity load description is required",
              "code": "field-validation-error",
              "fieldReference": "permitData.permittedCommodity.loadDescription"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "axleCalcViolations",
              "operator": "isEmptyArray",
              "value": true
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle configuration failed axle calculation policy checks",
              "code": "policy-validation-error",
              "details": {
                "fact": "axleCalcViolations"
              }
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "fixedCost",
          "params": {
            "cost": 15
          }
        }
      ],
      "conditions": []
    },
    {
      "id": "STFR",
      "name": "Single Trip ICBC Basic Insurance (FR)",
      "routingRequired": false,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "BUSCRUM",
        "BUSTRLR",
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "DDCKBUS",
        "GRADERS",
        "LCVRMDB",
        "LCVTPDB",
        "LOGGING",
        "LOGOFFH",
        "MUNFITR",
        "OGBEDTK",
        "OGOILSW",
        "OGSERVC",
        "OGSRRAH",
        "PICKRTT",
        "PLOWBLD",
        "PUTAXIS",
        "REGTRCK",
        "SCRAPER",
        "STINGER",
        "TOWVEHC",
        "TRKTRAC"
      ],
      "rules": [
        {
          "conditions": {
            "any": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "lessThanInclusive",
                  "value": 30
                }
              },
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "greaterThan",
                  "value": 0
                }
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Duration must be 30 days or less",
              "code": "field-validation-error",
              "fieldReference": "permitData.permitDuration"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.licensedGVW",
              "operator": "greaterThan",
              "value": 0
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Licensed GVW must be greater than zero",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.licensedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.licensedGVW",
                "operator": "greaterThan",
                "value": 63500
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Licensed GVW may not exceed 63,500 kg",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.licensedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.countryCode",
                "operator": "equal",
                "value": "CA"
              },
              {
                "fact": "permitData",
                "path": "vehicleDetails.provinceCode",
                "operator": "equal",
                "value": "BC"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "BC plated vehicles do not require this permit",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.provinceCode"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "thirdPartyLiability",
              "operator": "in",
              "value": [
                "GENERAL_GOODS",
                "DANGEROUS_GOODS"
              ]
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Third party liability is invalid",
              "code": "field-validation-error",
              "fieldReference": "permitData.thirdPartyLiability"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "fixedCost",
          "params": {
            "cost": 84
          }
        },
        {
          "fact": "conditionalFixedCost",
          "params": {
            "cost": 5,
            "fact": "thirdPartyLiability",
            "value": "DANGEROUS_GOODS"
          }
        }
      ]
    },
    {
      "id": "QRFR",
      "name": "Quarterly ICBC Basic Insurance (FR)",
      "routingRequired": false,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "BUSCRUM",
        "BUSTRLR",
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "DDCKBUS",
        "GRADERS",
        "LCVRMDB",
        "LCVTPDB",
        "LOGGING",
        "LOGOFFH",
        "MUNFITR",
        "OGBEDTK",
        "OGOILSW",
        "OGSERVC",
        "OGSRRAH",
        "PICKRTT",
        "PLOWBLD",
        "PUTAXIS",
        "REGTRCK",
        "SCRAPER",
        "STINGER",
        "TOWVEHC",
        "TRKTRAC"
      ],
      "rules": [
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "expiryDate",
              "operator": "equal",
              "value": {
                "fact": "endOfPermitQuarter"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Permit end date must be the end of the quarter in which the permit starts",
              "code": "field-validation-error",
              "fieldReference": "permitData.expiryDate"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.licensedGVW",
              "operator": "greaterThan",
              "value": 0
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Licensed GVW must be greater than zero",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.licensedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.licensedGVW",
                "operator": "greaterThan",
                "value": 63500
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Licensed GVW may not exceed 63,500 kg",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.licensedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.countryCode",
                "operator": "equal",
                "value": "CA"
              },
              {
                "fact": "permitData",
                "path": "vehicleDetails.provinceCode",
                "operator": "equal",
                "value": "BC"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "BC plated vehicles do not require this permit",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.provinceCode"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "operator": "in",
              "value": [
                "GENERAL_GOODS",
                "DANGEROUS_GOODS"
              ],
              "path": "thirdPartyLiability"
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Third party liability is invalid",
              "code": "field-validation-error",
              "fieldReference": "permitData.thirdPartyLiability"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "fixedCost",
          "params": {
            "cost": 843
          }
        },
        {
          "fact": "conditionalFixedCost",
          "params": {
            "cost": 56,
            "fact": "thirdPartyLiability",
            "value": "DANGEROUS_GOODS"
          }
        }
      ]
    },
    {
      "id": "MFP",
      "name": "Motive Fuel User Permit",
      "routingRequired": true,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "BUSCRUM",
        "BUSTRLR",
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "DDCKBUS",
        "GRADERS",
        "LCVRMDB",
        "LCVTPDB",
        "LOGGING",
        "LOGOFFH",
        "OGBEDTK",
        "OGOILSW",
        "OGSERVC",
        "OGSRRAH",
        "PICKRTT",
        "PLOWBLD",
        "REGTRCK",
        "SCRAPER",
        "STINGER",
        "TOWVEHC",
        "TRKTRAC"
      ],
      "rules": [
        {
          "conditions": {
            "any": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "lessThanInclusive",
                  "value": 7
                }
              },
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "greaterThan",
                  "value": 0
                }
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Duration must be 7 days or less",
              "code": "field-validation-error",
              "fieldReference": "permitData.permitDuration"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.licensedGVW",
              "operator": "greaterThan",
              "value": 0
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Licensed GVW must be greater than zero",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.licensedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.licensedGVW",
                "operator": "greaterThan",
                "value": 63500
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Licensed GVW may not exceed 63,500 kg",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.licensedGVW"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "permittedRoute.manualRoute.origin",
              "operator": "stringMinimumLength",
              "value": 1
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Route origin is required",
              "code": "field-validation-error",
              "fieldReference": "permitData.permittedRoute.manualRoute.origin"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "permittedRoute.manualRoute.destination",
              "operator": "stringMinimumLength",
              "value": 1
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Route destination is required",
              "code": "field-validation-error",
              "fieldReference": "permitData.permittedRoute.manualRoute.origin"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "permittedRoute.manualRoute.totalDistance",
              "operator": "greaterThan",
              "value": 0
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Trip distance must be greater than zero",
              "code": "field-validation-error",
              "fieldReference": "permitData.permittedRoute.manualRoute.totalDistance"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "costPerKilometre",
          "params": {
            "cost": 0.07,
            "minValue": 10,
            "maxValue": 140
          }
        }
      ]
    },
    {
      "id": "NRSCV",
      "name": "Non-Resident Single Trip",
      "routingRequired": false,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "BUSCRUM",
        "BUSTRLR",
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "DDCKBUS",
        "FARMTRC",
        "GRADERS",
        "LCVRMDB",
        "LCVTPDB",
        "LOGGING",
        "LOGOFFH",
        "MUNFITR",
        "OGBEDTK",
        "OGOILSW",
        "OGSERVC",
        "OGSRRAH",
        "PICKRTT",
        "PLOWBLD",
        "PUTAXIS",
        "REGTRCK",
        "SCRAPER",
        "STINGER",
        "TOWVEHC",
        "TRKTRAC"
      ],
      "rules": [
        {
          "conditions": {
            "any": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "lessThanInclusive",
                  "value": 30
                }
              },
              {
                "not": {
                  "fact": "permitData",
                  "path": "permitDuration",
                  "operator": "greaterThan",
                  "value": 0
                }
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Duration must be 30 days or less",
              "code": "field-validation-error",
              "fieldReference": "permitData.permitDuration"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "vehicleConfiguration.loadedGVW",
                  "operator": "greaterThan",
                  "value": 0
                }
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "in",
                "value": [
                  "conditional",
                  "none",
                  "x-plated"
                ]
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Loaded GVW must be greater than zero for the selected conditional license fee type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.loadedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "vehicleConfiguration.netWeight",
                  "operator": "greaterThan",
                  "value": 0
                }
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "equal",
                "value": "farm-vehicle"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Net weight must be greater than zero for a farm vehicle",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.netWeight"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleConfiguration.netWeight",
                "operator": "greaterThan",
                "value": 24400
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "equal",
                "value": "farm-vehicle"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Net weight may not exceed 24,400 kg for a farm vehicle",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.netWeight"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleConfiguration.loadedGVW",
                "operator": "greaterThan",
                "value": 63500
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "notEqual",
                "value": "farm-vehicle"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Loaded GVW may not exceed 63,500 kg",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.loadedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.countryCode",
                "operator": "equal",
                "value": "CA"
              },
              {
                "fact": "permitData",
                "path": "vehicleDetails.provinceCode",
                "operator": "equal",
                "value": "BC"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "BC plated vehicles do not require this permit",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.provinceCode"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "conditionalLicensingFee",
              "operator": "in",
              "value": [
                "none",
                "conditional",
                "x-plated",
                "farm-vehicle",
                "farm-tractor"
              ]
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Conditional licensing selection is invalid",
              "code": "field-validation-error",
              "fieldReference": "permitData.conditionalLicensingFee"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "conditionalFixedCost",
          "params": {
            "cost": 12,
            "fact": "conditionalLicensingFee",
            "value": "conditional"
          }
        },
        {
          "fact": "conditionalFixedCost",
          "params": {
            "cost": 33,
            "fact": "conditionalLicensingFee",
            "value": "farm-tractor"
          }
        },
        {
          "fact": "rangeMatrixCostLookup",
          "params": {
            "divisor": 12,
            "matrixMap": [
              {
                "key": "none",
                "value": "annualFeeCV"
              },
              {
                "key": "x-plated",
                "value": "annualFeeIndustrial"
              }
            ],
            "rangeLookupKey": "conditionalLicensingFee",
            "matrixFactValue": "vehicleConfiguration.loadedGVW"
          }
        },
        {
          "fact": "rangeMatrixCostLookup",
          "params": {
            "divisor": 12,
            "matrixMap": [
              {
                "key": "farm-vehicle",
                "value": "annualFeeFarm"
              }
            ],
            "rangeLookupKey": "conditionalLicensingFee",
            "matrixFactValue": "vehicleConfiguration.netWeight"
          }
        }
      ]
    },
    {
      "id": "NRQCV",
      "name": "Non-Resident Quarterly",
      "routingRequired": false,
      "weightDimensionRequired": false,
      "sizeDimensionRequired": false,
      "commodityRequired": false,
      "allowedVehicles": [
        "BUSCRUM",
        "BUSTRLR",
        "CONCRET",
        "CRANEAT",
        "CRANEMB",
        "DDCKBUS",
        "FARMTRC",
        "GRADERS",
        "LCVRMDB",
        "LCVTPDB",
        "LOGGING",
        "LOGOFFH",
        "MUNFITR",
        "OGBEDTK",
        "OGOILSW",
        "OGSERVC",
        "OGSRRAH",
        "PICKRTT",
        "PLOWBLD",
        "PUTAXIS",
        "REGTRCK",
        "SCRAPER",
        "STINGER",
        "TOWVEHC",
        "TRKTRAC"
      ],
      "rules": [
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "expiryDate",
              "operator": "equal",
              "value": {
                "fact": "endOfPermitQuarter"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Permit end date must be the end of the quarter in which the permit starts",
              "code": "field-validation-error",
              "fieldReference": "permitData.expiryDate"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "vehicleConfiguration.loadedGVW",
                  "operator": "greaterThan",
                  "value": 0
                }
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "in",
                "value": [
                  "conditional",
                  "none",
                  "x-plated"
                ]
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Loaded GVW must be greater than zero for the selected conditional license fee type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.loadedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "not": {
                  "fact": "permitData",
                  "path": "vehicleConfiguration.netWeight",
                  "operator": "greaterThan",
                  "value": 0
                }
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "equal",
                "value": "farm-vehicle"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Net weight must be greater than zero for a farm vehicle",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.netWeight"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleConfiguration.netWeight",
                "operator": "greaterThan",
                "value": 24400
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "equal",
                "value": "farm-vehicle"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Net weight may not exceed 24,400 kg for a farm vehicle",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.netWeight"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleConfiguration.loadedGVW",
                "operator": "greaterThan",
                "value": 63500
              },
              {
                "fact": "permitData",
                "path": "conditionalLicensingFee",
                "operator": "notEqual",
                "value": "farm-vehicle"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Loaded GVW may not exceed 63,500 kg",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleConfiguration.loadedGVW"
            }
          }
        },
        {
          "conditions": {
            "all": [
              {
                "fact": "permitData",
                "path": "vehicleDetails.countryCode",
                "operator": "equal",
                "value": "CA"
              },
              {
                "fact": "permitData",
                "path": "vehicleDetails.provinceCode",
                "operator": "equal",
                "value": "BC"
              }
            ]
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "BC plated vehicles do not require this permit",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.provinceCode"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "vehicleDetails.vehicleSubType",
              "operator": "in",
              "value": {
                "fact": "allowedVehicles"
              }
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Vehicle type not permittable for this permit type",
              "code": "field-validation-error",
              "fieldReference": "permitData.vehicleDetails.vehicleSubType"
            }
          }
        },
        {
          "conditions": {
            "not": {
              "fact": "permitData",
              "path": "conditionalLicensingFee",
              "operator": "in",
              "value": [
                "none",
                "conditional",
                "x-plated",
                "farm-vehicle",
                "farm-tractor"
              ]
            }
          },
          "event": {
            "type": "violation",
            "params": {
              "message": "Conditional licensing selection is invalid",
              "code": "field-validation-error",
              "fieldReference": "permitData.conditionalLicensingFee"
            }
          }
        }
      ],
      "costRules": [
        {
          "fact": "conditionalFixedCost",
          "params": {
            "cost": 12,
            "fact": "conditionalLicensingFee",
            "value": "conditional"
          }
        },
        {
          "fact": "conditionalFixedCost",
          "params": {
            "cost": 33,
            "fact": "conditionalLicensingFee",
            "value": "farm-tractor"
          }
        },
        {
          "fact": "rangeMatrixCostLookup",
          "params": {
            "divisor": 4,
            "matrixMap": [
              {
                "key": "none",
                "value": "annualFeeCV"
              },
              {
                "key": "x-plated",
                "value": "annualFeeIndustrial"
              }
            ],
            "rangeLookupKey": "conditionalLicensingFee",
            "matrixFactValue": "vehicleConfiguration.loadedGVW"
          }
        },
        {
          "fact": "rangeMatrixCostLookup",
          "params": {
            "divisor": 4,
            "matrixMap": [
              {
                "key": "farm-vehicle",
                "value": "annualFeeFarm"
              }
            ],
            "rangeLookupKey": "conditionalLicensingFee",
            "matrixFactValue": "vehicleConfiguration.netWeight"
          }
        }
      ]
    }
  ],
  "globalWeightDefaults": {
    "powerUnits": [
      {
        "axles": 11,
        "saLegal": 6000,
        "saPermittable": 9100,
        "daLegal": 9100,
        "daPermittable": 11000
      },
      {
        "axles": 12,
        "saLegal": 6000,
        "saPermittable": 9100,
        "daLegal": 17000,
        "daPermittable": 23000
      },
      {
        "axles": 13,
        "saLegal": 7300,
        "saPermittable": 9100,
        "daLegal": 24000,
        "daPermittable": 28000
      },
      {
        "axles": 22,
        "saLegal": 17000,
        "saPermittable": 17000,
        "daLegal": 17000,
        "daPermittable": 23000
      },
      {
        "axles": 23,
        "saLegal": 13600,
        "saPermittable": 15200,
        "daLegal": 24000,
        "daPermittable": 28000
      }
    ],
    "trailers": [
      {
        "axles": 1,
        "legal": 9100,
        "permittable": 11000
      },
      {
        "axles": 2,
        "legal": 17000,
        "permittable": 23000
      },
      {
        "axles": 3,
        "legal": 24000,
        "permittable": 28000
      }
    ]
  },
  "vehicleCategories": {
    "trailerCategories": [
      {
        "id": "trailer",
        "name": "Default trailer category"
      },
      {
        "id": "accessory",
        "name": "Accessory trailer such as jeep or booster, to be used alongside other trailers. Not permittable on its own as a trailer in a combination."
      },
      {
        "id": "pseudo",
        "name": "Placeholder for a trailer that is not technically a separate trailer, such as (None) to represent no trailer in a combination, or an extra axle."
      },
      {
        "id": "semi",
        "name": "Semi-Trailer",
        "defaultWeightDimensions": [
          {
            "axles": 3,
            "legal": 24000,
            "permittable": 29000
          },
          {
            "axles": 3,
            "modifier": {
              "type": "BOOSTER",
              "position": "after",
              "axles": 2
            },
            "legal": 24000,
            "permittable": 28000
          },
          {
            "axles": 3,
            "modifier": {
              "type": "BOOSTER",
              "position": "after",
              "axles": 3
            },
            "legal": 24000,
            "permittable": 28000
          }
        ]
      },
      {
        "id": "wheeler",
        "name": "Wheeler",
        "defaultWeightDimensions": [
          {
            "axles": 1,
            "legal": 0,
            "permittable": 0
          },
          {
            "axles": 2,
            "legal": 17000,
            "permittable": 31000
          },
          {
            "axles": 3,
            "legal": 24000,
            "permittable": 40000
          }
        ]
      },
      {
        "id": "steering",
        "name": "Steering Trailer"
      },
      {
        "id": "dolly",
        "name": "Dolly"
      },
      {
        "id": "pony",
        "name": "Pony Trailer",
        "defaultWeightDimensions": [
          {
            "axles": 2,
            "legal": 17000,
            "permittable": 21000
          },
          {
            "axles": 3,
            "legal": 21000,
            "permittable": 21000
          }
        ]
      }
    ],
    "powerUnitCategories": [
      {
        "id": "powerunit",
        "name": "Default power unit category"
      },
      {
        "id": "multisteer",
        "name": "Power units supporting greater than tandem steer axle units",
        "defaultWeightDimensions": [
          {
            "axles": 11,
            "saLegal": 9100,
            "saPermittable": 9100,
            "daLegal": 9100,
            "daPermittable": 11000
          },
          {
            "axles": 12,
            "saLegal": 9100,
            "saPermittable": 12000,
            "daLegal": 17000,
            "daPermittable": 23000
          },
          {
            "axles": 13,
            "saLegal": 9100,
            "saPermittable": 12000,
            "daLegal": 24000,
            "daPermittable": 28000
          },
          {
            "axles": 23,
            "saLegal": 15200,
            "saPermittable": 17000,
            "daLegal": 24000,
            "daPermittable": 28000
          },
          {
            "axles": 33,
            "saLegal": 24000,
            "saPermittable": 24000,
            "daLegal": 24000,
            "daPermittable": 28000
          }
        ]
      }
    ]
  },
  "vehicleTypes": {
    "powerUnitTypes": [
      {
        "id": "BUSCRUM",
        "name": "Buses/Crummies",
        "category": "powerunit"
      },
      {
        "id": "CONCRET",
        "name": "Concrete Pumper Trucks",
        "category": "multisteer",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "CRANEAT",
        "name": "Cranes - All Terrain",
        "category": "multisteer",
        "additionalAxleSubType": "ATCAXLE",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "CRANEMB",
        "name": "Cranes - Mobile",
        "category": "multisteer",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "CRAFTAT",
        "name": "Cranes, Rubber-Tired Loaders, Firetrucks - All Terrain",
        "category": "powerunit",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "CRAFTMB",
        "name": "Cranes, Rubber-Tired Loaders, Firetrucks - Mobile",
        "category": "powerunit",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "DDCKBUS",
        "name": "Double Decker Buses",
        "category": "powerunit"
      },
      {
        "id": "FARMTRC",
        "name": "Farm Tractors",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "FARMVEH",
        "name": "Farm Vehicles",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "GRADERS",
        "name": "Fixed Equipment - Trucks/Graders etc.",
        "category": "powerunit",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "BUSTRLR",
        "name": "Intercity Buses (Pulling Pony Trailers)",
        "category": "powerunit"
      },
      {
        "id": "LOGGING",
        "name": "Logging Trucks",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "LOGOFFH",
        "name": "Logging Trucks - Off-Highway",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "LCVRMDB",
        "name": "Long Combination Vehicles (LCV) - Rocky Mountain Doubles",
        "category": "powerunit",
        "isLcv": true,
        "conditions": [
          {
            "condition": "CVSE-1014",
            "mandatory": true
          }
        ],
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "LCVTPDB",
        "name": "Long Combination Vehicles (LCV) - Turnpike Doubles",
        "category": "powerunit",
        "isLcv": true,
        "conditions": [
          {
            "condition": "CVSE-1014",
            "mandatory": true
          }
        ],
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "MUNFITR",
        "name": "Municipal Fire Trucks",
        "category": "multisteer",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "OGBEDTK",
        "name": "Oil and Gas - Bed Trucks",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "OGOILSW",
        "name": "Oil and Gas - Oilfield Sows",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "OGSERVC",
        "name": "Oil and Gas - Service Rigs",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "OGSRRAH",
        "name": "Oil and Gas - Service Rigs and Rathole Augers Only Equipped with Heavy Front Projected Crane (must exceed 14,000 kg tare weight)",
        "category": "multisteer",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "PICKRTT",
        "name": "Picker Truck Tractors",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "PICKRTR",
        "name": "Picker Trucks",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "RBTRLDR",
        "name": "Rubber Tired Loaders",
        "category": "multisteer",
        "displayCodePrefix": "MC",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "A"
      },
      {
        "id": "SCRAPER",
        "name": "Scrapers",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "SPAUTHV",
        "name": "Specially Authorized Vehicles",
        "category": "powerunit"
      },
      {
        "id": "PUTAXIS",
        "name": "Taxis",
        "category": "powerunit"
      },
      {
        "id": "TLSCONV",
        "name": "Telescopic Conveyor Trucks",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "TOWVEHC",
        "name": "Tow Vehicles",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "TRKTRAC",
        "name": "Truck Tractors",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "STINGER",
        "name": "Truck Tractors - Stinger Steered",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "REGTRCK",
        "name": "Trucks",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      },
      {
        "id": "PLOWBLD",
        "name": "Trucks Equipped with Front or Underbody Plow Blades",
        "category": "powerunit",
        "displayCodePrefix": "TT",
        "displayCodeSteerAxle": "S",
        "displayCodeDriveAxle": "D"
      }
    ],
    "trailerTypes": [
      {
        "id": "PFMAXLE",
        "name": "Additional Axle Unit for Platform Trailer",
        "category": "pseudo",
        "ignoreForSizeDimensions": true,
        "displayCode": "A"
      },
      {
        "id": "ATCAXLE",
        "name": "Additional Axle Unit for All-Terrain Crane",
        "category": "pseudo",
        "ignoreForSizeDimensions": true,
        "displayCode": "A"
      },
      {
        "id": "BOOSTER",
        "name": "Boosters",
        "category": "accessory",
        "ignoreForSizeDimensions": true,
        "defaultWeightDimensions": [
          {
            "axles": 1,
            "legal": 9100,
            "permittable": 11000
          },
          {
            "axles": 1,
            "modifier": {
              "category": "semi",
              "position": "before",
              "axles": 3,
              "minInterAxleSpacing": 0,
              "maxInterAxleSpacing": 419
            },
            "legal": 9100,
            "permittable": 9100
          }
        ],
        "displayCode": "B"
      },
      {
        "id": "DOLLIES",
        "name": "Dollies",
        "category": "dolly",
        "displayCode": "B"
      },
      {
        "id": "EXPANDO",
        "name": "Expando Semi-Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "FECVYRX",
        "name": "Fixed Equipment - Conveyors",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "FEDRMMX",
        "name": "Fixed Equipment - Counter Flow Asphalt Drum Mixers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "FEPNYTR",
        "name": "Fixed Equipment - Pony Trailers",
        "category": "pony",
        "displayCode": "B"
      },
      {
        "id": "FEBGHSE",
        "name": "Fixed Equipment - Portable Asphalt Baghouses",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "FESEMTR",
        "name": "Fixed Equipment - Semi-Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "FEWHELR",
        "name": "Fixed Equipment - Wheeler Semi-Trailers",
        "category": "wheeler",
        "displayCode": "T"
      },
      {
        "id": "FEWHLPR",
        "name": "Fixed Equipment - Semi Trailer (16 and 24 Wheeler) Peace River Only",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "FLOATTR",
        "name": "Float Trailers",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "FULLLTL",
        "name": "Full Trailers",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "JEEPSRG",
        "name": "Jeeps",
        "category": "accessory",
        "ignoreForSizeDimensions": true,
        "displayCode": "J"
      },
      {
        "id": "LOGNTAC",
        "name": "Legacy Logging Trailer Combinations - Non-TAC B-Trains",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGTNDM",
        "name": "Legacy Logging Trailer Combinations - Single Axle Jeeps, Tandem Axle Pole Trailers, Dogloggers",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGTRIX",
        "name": "Legacy Logging Trailer Combinations - Single Axle Jeeps, Tri Axle Trailers",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGDGLG",
        "name": "Legacy Logging Trailer Combinations - Tandem Pole Trailers, Dogloggers",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGSMEM",
        "name": "Logging Semi-Trailer - Empty, 3.2 m Bunks",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGFULL",
        "name": "Logging Trailer - Full Trailers, Tri Axle, Quad Axle",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGOWBK",
        "name": "Logging Trailers - Overwidth Bunks",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "MHMBSHG",
        "name": "Manufactured Homes, Modular Buildings, Structures and Houseboats (> 5.0 m OAW) with Attached Axles",
        "category": "pony",
        "displayCode": "T"
      },
      {
        "id": "MHMBSHL",
        "name": "Manufactured Homes, Modular Buildings, Structures and Houseboats (<= 5.0 m OAW) with Attached Axles",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "OGOSFDT",
        "name": "Oil and Gas - Oversize Oilfield Flat Deck Semi-Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "ODTRLEX",
        "name": "Overdimensional Trailers and Semi-Trailers (For Export)",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "PMHWAAX",
        "name": "Park Model Homes with Attached Axles",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "PLATFRM",
        "name": "Platform Trailers",
        "category": "semi",
        "additionalAxleSubType": "PFMAXLE",
        "displayCode": "B"
      },
      {
        "id": "POLETRL",
        "name": "Pole Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "PONYTRL",
        "name": "Pony Trailers",
        "category": "pony",
        "displayCode": "B"
      },
      {
        "id": "REDIMIX",
        "name": "Ready Mix Concrete Pump Semi-Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "SEMITRL",
        "name": "Semi-Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "STACTRN",
        "name": "Semi-Trailers - A-Trains and C-Trains",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "STBTRAN",
        "name": "Semi-Trailers - B-Trains",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "HIBOEXP",
        "name": "Semi-Trailers - Hiboys/Expandos",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "HIBOFLT",
        "name": "Semi-Trailers - Hiboys/Flat Decks",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "STRDLY",
        "name": "Steering Dollies",
        "category": "trailer",
        "displayCode": "B"
      },
      {
        "id": "STREEFR",
        "name": "Semi-Trailers - Insulated Vans with Reefer/Refrigeration Units",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "STLOGNG",
        "name": "Semi-Trailers - Logging",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "STNTSHC",
        "name": "Semi-Trailers - Non-Tac Short Chassis",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "STSDBDK",
        "name": "Semi-Trailers - Single Drop, Double Drop, Step Decks, Lowbed, Expandos, etc.",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "STWDTAN",
        "name": "Semi-Trailers - Spread Tandems",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "STSTEER",
        "name": "Semi-Trailers - Steering Trailers",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "STSTNGR",
        "name": "Semi-Trailers - Stinger Steered Automobile Transporters",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "STCHIPS",
        "name": "Semi-Trailers - Walled B-Trains (Chip Trucks)",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "STWHELR",
        "name": "Semi-Trailers - Wheelers",
        "category": "wheeler",
        "displayCode": "T"
      },
      {
        "id": "STWIDWH",
        "name": "Semi-Trailers - Wide Wheelers",
        "category": "wheeler",
        "displayCode": "T"
      },
      {
        "id": "STCRANE",
        "name": "Semi-Trailers with Crane",
        "category": "semi",
        "displayCode": "T"
      },
      {
        "id": "STROPRT",
        "name": "Steering Trailers - Manned",
        "category": "steering",
        "displayCode": "T"
      },
      {
        "id": "STRSELF",
        "name": "Steering Trailers - Self/Remote",
        "category": "steering",
        "displayCode": "T"
      },
      {
        "id": "STINGAT",
        "name": "Stinger Steered Automobile Transporters",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "DBTRBTR",
        "name": "Tandem/Tridem Drive B-Train (Super B-Train)",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "LOGLGCY",
        "name": "Legacy Logging Trailer Combinations",
        "category": "trailer",
        "displayCode": "T"
      },
      {
        "id": "XXXXXXX",
        "name": "None",
        "category": "pseudo",
        "ignoreForAxleCalculation": true
      }
    ]
  },
  "commodities": [
    {
      "id": "XXXXXXX",
      "name": "None",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "FEDRMMX",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.72,
                  "l": 31,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "FEBGHSE",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 4.26,
                  "h": 4.72,
                  "l": 31,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "FECVYRX",
              "jeep": true,
              "booster": false,
              "weightPermittable": true
            },
            {
              "type": "FESEMTR",
              "jeep": true,
              "booster": true,
              "weightPermittable": true
            },
            {
              "type": "ODTRLEX",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.65,
                  "h": 4.15,
                  "l": 23
                }
              ]
            },
            {
              "type": "REDIMIX",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 31,
                  "regions": [
                    {
                      "region": "LMN",
                      "l": 23
                    },
                    {
                      "region": "ELK",
                      "l": 23
                    },
                    {
                      "region": "YHO",
                      "l": 23
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STREEFR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 23
                }
              ]
            },
            {
              "type": "STNTSHC",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 5,
                  "rp": 2.25,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 20
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "l": 32
                    }
                  ]
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 40
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "STRSELF",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 36
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "FEWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 31,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "GRADERS",
          "trailers": [
            {
              "type": "FEPNYTR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 31
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.5,
                  "h": 4.4,
                  "l": 12.5,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.3
                    },
                    {
                      "region": "ELK",
                      "h": 4.3
                    },
                    {
                      "region": "YHO",
                      "h": 4.3
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "CRANEAT",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 25
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 14,
                  "regions": [
                    {
                      "region": "PCE",
                      "l": 15
                    }
                  ]
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "MUNFITR",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 25
                }
              ]
            }
          ]
        },
        {
          "type": "RBTRLDR",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 25
                }
              ]
            }
          ]
        },
        {
          "type": "CRANEMB",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 25
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 14,
                  "regions": [
                    {
                      "region": "PCE",
                      "l": 15
                    }
                  ]
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "FEPNYTR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 31
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "MHMBSHL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 5,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "MHMBSHG",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 6,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 6.1,
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 16,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.4
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "ODTRLEX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.65,
                  "h": 4.15,
                  "l": 23
                }
              ]
            },
            {
              "type": "PMHWAAX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.9,
                  "w": 4.4,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TLSCONV",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 15.5
                }
              ]
            }
          ]
        },
        {
          "type": "CONCRET",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 15.5
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "DDCKBUS",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.42,
                  "l": 12.5,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.3
                    },
                    {
                      "region": "ELK",
                      "h": 4.3
                    },
                    {
                      "region": "YHO",
                      "h": 4.3
                    },
                    {
                      "region": "PCE",
                      "h": 4.3
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "PICKRTT",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 16
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "STCRANE",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 25
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 40,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "STRSELF",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 36,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "SCRAPER",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 12.5
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "PLOWBLD",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 12.5
                }
              ]
            }
          ]
        },
        {
          "type": "BUSTRLR",
          "trailers": [
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 20
                }
              ]
            }
          ]
        },
        {
          "type": "LCVRMDB",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 32,
                  "regions": [
                    {
                      "region": "PCE",
                      "l": 31
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "LCVTPDB",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 41
                }
              ]
            }
          ]
        },
        {
          "type": "LOGOFFH",
          "trailers": [
            {
              "type": "STLOGNG",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 4.4,
                  "h": 4.15,
                  "l": 23
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "DOGLOGG",
      "name": "Doglogger/Sjostrum Trailers (decked)",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 13.5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "GRTBBUK",
      "name": "Grader, Tractor Blades, Buckets",
      "powerUnits": [
        {
          "type": "GRADERS",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 0,
                  "rp": 0,
                  "w": 4.4,
                  "h": 4.15,
                  "l": 12.5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "HAYRACK",
      "name": "Hayrack Semi-Trailer with a Folded Chassis/Empty Piggyback",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STLOGNG",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "IMCONTN",
      "name": "Intermodal Containers",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 12.5
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 26
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 23
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "IMCONWS",
      "name": "Intermodal Containers without Sides",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 4.4,
                  "h": 4.72,
                  "l": 23
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "LPBOOMS",
      "name": "Logs, Poles And Boomsticks (Up To 20.1)",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "LOGLGCY",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 25
                }
              ]
            },
            {
              "type": "POLETRL",
              "jeep": true,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 26
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 25
                }
              ]
            },
            {
              "type": "STLOGNG",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 25
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "LPBOOML",
      "name": "Logs, Poles And Boomsticks (Over 20.1)",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "LOGFULL",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 10,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 40
                }
              ]
            },
            {
              "type": "POLETRL",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 8,
                  "rp": 9,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 40
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 10,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 40
                }
              ]
            },
            {
              "type": "STRSELF",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 10,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 36
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "MFHOMES",
      "name": "Manufactured Homes, Modular Buildings, Structures and Houseboats (<= 5.0 m OAW)",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 5,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "FLOATTR",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 5,
                  "h": 4.57,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 5,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 7.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 7.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "MFHOMEL",
      "name": "Manufactured Homes, Modular Buildings, Structures and Houseboats (> 5.0 m OAW)",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 6,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 6.1,
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "FLOATTR",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 6,
                  "h": 4.57,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 6.1,
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.9,
                  "w": 6,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 6.1,
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 7.5,
                  "w": 6,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 6.1,
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 7.5,
                  "w": 6,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 6.1,
                      "h": 5.33,
                      "l": 36
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "PARKMHS",
      "name": "Park Model Homes",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.9,
                  "w": 4.4,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FLOATTR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.9,
                  "w": 4.4,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.9,
                  "w": 4.4,
                  "h": 4.88,
                  "l": 31.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "PIPESTL",
      "name": "Pipe And Steel Products (Rebar, Pilings, Reinforcing Steel, Etc.)",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 31
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "STSTEER",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 36
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 40
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "REDUCBL",
      "name": "Reducible Loads",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWIDWH",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "PLATFRM",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "SEMITRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 26,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STLOGNG",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSTEER",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STCRANE",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 40,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STRSELF",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 36,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.4
                    }
                  ]
                }
              ]
            },
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.4
                    }
                  ]
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 16,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.4
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.4
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "PICKRTT",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "STINGER",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1.2,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSTNGR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1.2,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "SCRAPER",
      "name": "Scraper on Dollies",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": true,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.4
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "OILFILD",
      "name": "Oil Field Equipment",
      "powerUnits": [
        {
          "type": "OGBEDTK",
          "trailers": [
            {
              "type": "EXPANDO",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 3.3,
                  "h": 4.3,
                  "l": 14
                }
              ]
            },
            {
              "type": "OGOSFDT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.3,
                  "h": 4.3,
                  "l": 23
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "OGOILSW",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 15
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 25
                }
              ]
            }
          ]
        },
        {
          "type": "OGSERVC",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 15
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.3,
                  "l": 23
                }
              ]
            }
          ]
        },
        {
          "type": "OGSRRAH",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 15.5
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 10,
                  "rp": 6.5,
                  "w": 2.9,
                  "h": 4.15,
                  "l": 23
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "OGOSFDT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.3,
                  "h": 4.3,
                  "l": 23
                }
              ],
              "weightPermittable": true
            }
          ]
        }
      ]
    },
    {
      "id": "JPTRLOG",
      "name": "Tandem Jeep/Pole Trailer Loaded on Logging Truck",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 2.9,
                  "h": 4.3,
                  "l": 12.5
                }
              ]
            }
          ]
        },
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 2.9,
                  "h": 4.3,
                  "l": 12.5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "TOWDISB",
      "name": "Tow Trucks And Disabled Vehicles",
      "powerUnits": [
        {
          "type": "TOWVEHC",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 27.5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "TRQDLOG",
      "name": "Tri-Axle or Quad Axle Full Trailer Loaded on Logging Truck",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 13.5
                }
              ]
            }
          ]
        },
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 5,
                  "w": 2.6,
                  "h": 4.3,
                  "l": 13.5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "WOODCHP",
      "name": "Wood Chips, Residuals",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.45,
                  "l": 27.5
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "EMPTYXX",
      "name": "Empty",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "LOGOWBK",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 0,
                  "rp": 0,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 23
                }
              ]
            },
            {
              "type": "PLATFRM",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 31,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.15
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.15
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.4,
                  "l": 31,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.15
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.15
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWIDWH",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.15
                    },
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "PICKRTT",
          "trailers": [
            {
              "type": "OGOSFDT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 23
                }
              ]
            },
            {
              "type": "PLATFRM",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 27.5
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "l": 27.5
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "STWIDWH",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            }
          ]
        },
        {
          "type": "PICKRTR",
          "trailers": [
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 0,
                  "rp": 0,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "GRBBINS",
      "name": "Garbage Bins",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 12.5
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "LAMBEAM",
      "name": "Laminated Beams",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "POLETRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 40
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 31
                }
              ],
              "weightPermittable": true
            }
          ]
        }
      ]
    },
    {
      "id": "HAYLREC",
      "name": "Hay Bales Large Rectangular",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 12.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 26,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.05,
                  "h": 4.4,
                  "l": 23,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.3
                    },
                    {
                      "region": "ELK",
                      "h": 4.3
                    },
                    {
                      "region": "YHO",
                      "h": 4.3
                    },
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "HAYROND",
      "name": "Hay Bales Round",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 12.5,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 26,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.5,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "YHO",
                      "h": 4.8
                    },
                    {
                      "region": "PCE",
                      "w": 3.84,
                      "h": 4.8
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "HAYSREC",
      "name": "Hay Bales Small Rectangular",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 12.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 26,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.05,
                  "h": 4.3,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 4.8
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "BRGBEAM",
      "name": "Bridge Beams",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "POLETRL",
              "jeep": true,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 31
                }
              ]
            },
            {
              "type": "SEMITRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 31,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STRDLY",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 36
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "NONREDU",
      "name": "Non-Reducible Loads",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "DOLLIES",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FULLLTL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 16,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            }
          ]
        },
        {
          "type": "PICKRTT",
          "trailers": [
            {
              "type": "OGOSFDT",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.3,
                  "h": 4.3,
                  "l": 23
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "SEMITRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.4,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.4,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSTEER",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STCRANE",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 40,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STRSELF",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 36,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWIDWH",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "PLATFRM",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ],
              "weightPermittable": true
            },
            {
              "type": "SEMITRL",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STACTRN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.4,
                  "l": 26,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STBTRAN",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.4,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOEXP",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.4,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "HIBOFLT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.4,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STLOGNG",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.15,
                  "l": 27.5
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSTEER",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STCRANE",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STROPRT",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 40,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STRSELF",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 36,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STWIDWH",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 5,
                  "h": 4.88,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "STINGER",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": false,
              "booster": false,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1.2,
                  "w": 2.6,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSTNGR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1.2,
                  "w": 2.6,
                  "h": 4.88,
                  "l": 25,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "AUTOCRR",
      "name": "Auto Carrier, Campers And Boats (Stinger Steered Transporters Only)",
      "powerUnits": [
        {
          "type": "STINGER",
          "trailers": [
            {
              "type": "STSTNGR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1.2,
                  "w": 2.6,
                  "h": 4.4,
                  "l": 25,
                  "regions": [
                    {
                      "region": "LMN",
                      "h": 4.3
                    },
                    {
                      "region": "ELK",
                      "h": 4.3
                    },
                    {
                      "region": "YHO",
                      "h": 4.3
                    },
                    {
                      "region": "PCE",
                      "h": 4.88
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "HAYRNPR",
      "name": "Hay Bales (Round) Peace River Only",
      "powerUnits": []
    },
    {
      "id": "BRSHCUT",
      "name": "Brushcutters (Peace Only)",
      "powerUnits": [
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "XXXXXXX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 12.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 4.57,
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "PONYTRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 1,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 3.8,
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "SEMITRL",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 4.57,
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "STSDBDK",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 2.6,
                  "h": 4.15,
                  "l": 23,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 4.57,
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        }
      ]
    },
    {
      "id": "FIXEDEQ",
      "name": "Fixed Equipment",
      "powerUnits": [
        {
          "type": "TRKTRAC",
          "trailers": [
            {
              "type": "FEWHLPR",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.2,
                  "h": 4.3,
                  "l": 27.5,
                  "regions": [
                    {
                      "region": "PCE",
                      "w": 3.8,
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FECVYRX",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 4,
                  "rp": 9.5,
                  "w": 3.8,
                  "h": 4.72,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FEDRMMX",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.72,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FEBGHSE",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 4.26,
                  "h": 4.72,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FESEMTR",
              "jeep": true,
              "booster": true,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.72,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FEWHELR",
              "jeep": true,
              "booster": true,
              "selfIssue": false,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.72,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            }
          ]
        },
        {
          "type": "REGTRCK",
          "trailers": [
            {
              "type": "FECVYRX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 4,
                  "rp": 9.5,
                  "w": 3.2,
                  "h": 4.3
                }
              ]
            },
            {
              "type": "FEDRMMX",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 3.8,
                  "h": 4.72,
                  "regions": [
                    {
                      "region": "PCE",
                      "h": 5.33
                    }
                  ]
                }
              ]
            },
            {
              "type": "FEPNYTR",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 1,
                  "rp": 4,
                  "w": 3.2,
                  "h": 4.3
                }
              ]
            },
            {
              "type": "FEBGHSE",
              "jeep": false,
              "booster": false,
              "selfIssue": true,
              "sizePermittable": true,
              "sizeDimensions": [
                {
                  "fp": 3,
                  "rp": 6.5,
                  "w": 4.26,
                  "h": 4.72
                }
              ]
            }
          ]
        }
      ]
    }
  ],
  "globalSizeDefaults": {
    "fp": 3,
    "rp": 6.5,
    "w": 2.6,
    "h": 4.15,
    "l": 31
  },
  "conditions": [
    {
      "description": "General Permit Conditions",
      "condition": "CVSE-1000",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1251"
    },
    {
      "description": "Permit Scope and Limitation",
      "condition": "CVSE-1070",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1261"
    },
    {
      "description": "Supplement for Structures",
      "condition": "CVSE-1000S",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1255"
    },
    {
      "description": "Log Permit Conditions",
      "condition": "CVSE-1000L",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1250"
    },
    {
      "description": "Highways and Restrictive Load Limits",
      "condition": "CVSE-1011",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1258"
    },
    {
      "description": "Routes Pre-Approved for 5.0 m OAW",
      "condition": "CVSE-1001",
      "conditionLink": ""
    },
    {
      "description": "General Permit Conditions to 6.1 m in the Peace River Area",
      "condition": "CVSE-1002",
      "conditionLink": ""
    },
    {
      "description": "East-West Overheight Corridors in the Lower Mainland",
      "condition": "CVSE-1010",
      "conditionLink": ""
    },
    {
      "description": "Routes - Woods Chips & Residual",
      "condition": "CVSE-1012",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1259"
    },
    {
      "description": "Restricted Routes for Hauling Wood on Wide Bunks",
      "condition": "CVSE-1013",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1254"
    },
    {
      "description": "LCV Operating Conditions & Routes",
      "condition": "CVSE-1014",
      "conditionLink": "https://www.th.gov.bc.ca/forms/getForm.aspx?formId=1260"
    }
  ],
  "standardTireSizes": [
    {
      "name": "254 (10\")",
      "size": 254
    },
    {
      "name": "279.4 (11\")",
      "size": 279
    },
    {
      "name": "304.8 (12\")",
      "size": 305
    },
    {
      "name": "315 (12.4\")",
      "size": 315
    },
    {
      "name": "325 (12.8\")",
      "size": 325
    },
    {
      "name": "330 (13\")",
      "size": 330
    },
    {
      "name": "355 (14\")",
      "size": 355
    },
    {
      "name": "365 (14.4\")",
      "size": 365
    },
    {
      "name": "368 (14.5\")",
      "size": 368
    },
    {
      "name": "381 (15\")",
      "size": 381
    },
    {
      "name": "385 (15.2\")",
      "size": 385
    },
    {
      "name": "393 (15.5\")",
      "size": 393
    },
    {
      "name": "406 (16\")",
      "size": 406
    },
    {
      "name": "425 (16.7\")",
      "size": 425
    },
    {
      "name": "431 (17\")",
      "size": 431
    },
    {
      "name": "445 (17.5\")",
      "size": 445
    },
    {
      "name": "457 (18\")",
      "size": 457
    },
    {
      "name": "502 (19.8\")",
      "size": 502
    },
    {
      "name": "508 (20\")",
      "size": 508
    },
    {
      "name": "520 (20.5\")",
      "size": 520
    },
    {
      "name": "525 (20.7\")",
      "size": 525
    },
    {
      "name": "550 (21.7\")",
      "size": 550
    },
    {
      "name": "609 (24\")",
      "size": 609
    },
    {
      "name": "622 (24.5\")",
      "size": 622
    },
    {
      "name": "636 (25\")",
      "size": 636
    },
    {
      "name": "711.2 (28\")",
      "size": 711.2
    }
  ],
  "vehicleDisplayCodeDefaults": {
    "prefixStandard": "",
    "prefixUniversal": "=",
    "paddingStandard": "-",
    "paddingUniversal": "=",
    "spacingUniversalDefault": "MU",
    "spacingUniversalSmall": "SU",
    "spacingUniversalLarge": "LU",
    "spacingUniversalSmallMax": 2,
    "spacingUniversalLargeMin": 5,
    "extraAxleUniversal": "XU",
    "endAxleUniversal": "EU",
    "maxAxlesStandard": 4,
    "thresholdAxlesUniversal": 4,
    "overAxlesCodeUniversal": "+",
    "universalAxleCode": "U",
    "multiDigitPrefix": "."
  }
}